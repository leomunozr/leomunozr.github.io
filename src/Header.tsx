        ...
        <header className={`${theme.colors.headerBg} backdrop-blur-md sticky top-0 z-50`}>
            <nav className="container mx-auto px-6 py-3 flex justify-end items-center">
                <div className="hidden md:flex md:px-30 space-x-6 items-center">
                    ...
                    <button onClick={toggleTheme} className={`py-2 rounded-full ${theme.colors.textSecondary} hover:${theme.colors.buttonPrimaryBg} ${theme.transition}`}>
                        {theme.name === 'dark' ? '☀️' : '🌙'}
                    </button>
                    <button onClick={toggleLanguage} className={`py-2 rounded-full ${theme.colors.textSecondary} hover:${theme.colors.buttonPrimaryBg} ${theme.transition}`} title="Translate">
